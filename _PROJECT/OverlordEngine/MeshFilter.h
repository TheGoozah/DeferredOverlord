...
private:
	...
	void BuildVertexBuffer(ID3D11Device* pDevice, UINT inputLayoutID, UINT inputLayoutSize, const std::vector<ILDescription>& inputLayoutDescriptions);
	void BuildVertexBuffer(const GameContext& gameContext, Material* pMaterial);
	void BuildIndexBuffer(const GameContext& gameContext) { BuildIndexBuffer(gameContext.pDevice); };
	...
	const VertexBufferData& GetVertexBufferData(UINT inputLayoutId);
