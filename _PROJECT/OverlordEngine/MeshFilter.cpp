...
void MeshFilter::BuildIndexBuffer(ID3D11Device* pDevice)
{
	...
	auto hr = pDevice->CreateBuffer(&bd, &initData, &m_pIndexBuffer);
...
{
	BuildVertexBuffer(gameContext.pDevice, pMaterial->m_InputLayoutID, pMaterial->m_pInputLayoutSize, pMaterial->m_pInputLayoutDescriptions);
}

void MeshFilter::BuildVertexBuffer(ID3D11Device* pDevice, UINT inputLayoutID, UINT inputLayoutSize, const std::vector<ILDescription>& inputLayoutDescriptions)
{
	...
	pDevice->CreateBuffer(&bd, &initData, &data.pVertexBuffer);
...
const VertexBufferData& MeshFilter::GetVertexBufferData(UINT inputLayoutId)
{
