...
void SceneManager::Initialize(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext, OverlordGame* pGame)
{
	...
	if (pGame->GetGameSettings().DirectX.DeferredRenderingEnabled)
...
void SceneManager::Draw() const
{
	...
	if (m_pGame->GetGameSettings().DirectX.DeferredRenderingEnabled)
	{
		DeferredRenderer::GetInstance()->Begin();
		m_ActiveScene->RootDraw();
		DeferredRenderer::GetInstance()->End(m_ActiveScene->m_GameContext);
